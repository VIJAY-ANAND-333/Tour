...
const name = localStorage.getItem('first_name')
  ...
  const sendNotification = async (id) => {
    try {
      ...
      console.log(obj);
      const response1 = await axios.post('http://localhost:4000/notification/add',obj);
    ...
    if (post.likes[0]?.userId[0]?._id!== userId) {
      ...
      try {
        const response = await axios.post(
          'http://localhost:4000/user/post/like/add',
          {
            userId: userId,
            postId: post._id,
      ...
      try {
        ...
        const response = await axios.delete(
          'http://localhost:4000/user/post/like/remove',
          {
            data: {
              userId: userId,
              postId: post._id
  ...
  {feed.userDetails?.[0]?.photo && (
    <img
      src={`http://localhost:4000/profile_images/${feed.userDetails[0].photo}`}
    ...
    />
        ...
        <CardMedia
          component="img"
          height="100%"
          image={`http://localhost:4000/posts/${feed.postUrl}`}
          alt={feed.caption}
        />
        <CardContent>
...
//             <Avatar sx={{ bgcolor: 'red', border: 'black' }} aria-label="recipe">
//               {feed.userDetails?.[0]?.photo ? (
//                 <img src={`http://localhost:4000/profile_images/${feed.userDetails[0].photo}`} alt="Avatar" />
//               ) : (
