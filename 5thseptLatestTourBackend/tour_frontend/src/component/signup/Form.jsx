...
const Form = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (isLogin) {
      ...
      const { data } = await axios.post(`http://localhost:4000/user/login`, { username: email, password })
      ...
      if (data) {
        localStorage.setItem("photo", `http://localhost:4000/profile_images/${data.data.photo}`);
      ...
      const { data } = await axios.post(`http://localhost:4000/user/create`, { first_name: name, last_name: last_name, mobile: mobile, photo: photo, email, password })
